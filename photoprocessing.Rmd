...
```{r}
# all_camera_data <- dplyr::bind_rows(data_2017, data_2018, data_2019, data_2020)
# 
# print(all_camera_data)
```
...
```{r}
BGW17 %>% pull(Count1Species) %>% length()

BGW17_drop_na <- BGW17 %>% tidyr::drop_na(Count1Species)

BGW17_drop_na %>% pull(Count1Species) %>% length()
```

```{r}
BGW17$Count1Species %in% BGW17_drop_na$Count1Species
```
...
# remove any rows where there was no detection for the 2019 data
...
# remove any rows where there was no detection for the 2020 data
...
```
***
We no longer need the raw data frames, so we can remove them from our environment just to clean things up a bit.
```{r}
rm(BGT18RAW, 
   BGW17RAW, 
   BGW18RAW, 
