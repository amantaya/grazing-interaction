...
```{r}
df <- data.frame(hour0 = (NA), hour1 = (NA), hour2 = (NA), hour3 = (NA), hour4 = (NA), hour5 = (NA), 
                 hour6 = (NA), hour7 = (NA), hour8 = (NA), hour9 = (NA), hour10 = (NA), hour11 = (NA),
                 hour12 = (NA), hour13 = (NA), hour14 = (NA), hour15 = (NA), hour16 = (NA), hour17 = (NA),
                 hour18 = (NA), hour19 = (NA), hour20 = (NA), hour21 = (NA), hour22 = (NA), hour23 = (NA))
df
```
...
```{r}
jpeg(filename = "figures/timeofday/BGW17_Counts_Small.jpg",
     width=5,
     height=5,
     units="in",
     res=300)
...
par(mar=c(2, 2, 1, 1))
plot(df$Freq,
     main = "Site Use by Time of Day",
     axes = FALSE,
     type = "n")
lines(x = df$hour, y = df$Freq)
points(x = df$hour, y = df$Freq, pch = 16)
axis(1, at = c(0:23), labels = c(0:23), cex.axis = 0.50)
axis(2, at = seq(0, 2000, by=500), cex.axis = 0.50)
dev.off()
...
```{r}
BGW17horses$hour <- as.factor(hour(BGW17horses$DateTime))

str(BGW17horses$hour)
```

```{r}
View(table(BGW17horses$hour))
barplot(table(BGW17horses$hour))
```

```{r}
BGW17group1 <- BGW17time[BGW17time$Group == 1,]
BGW17group1$hour <- hour(BGW17group1$DateTime)

BGW17group1

BGW17group2 <- BGW17time[BGW17time$Group == 2,]
BGW17group2$hour <- hour(BGW17group2$DateTime)

BGW17group2

hour <- unique(BGW17group1$hour)
hour <- as.factor(hour)
str(hour)
```
