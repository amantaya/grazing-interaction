...
grouped_site_dataframes <- csv_files_tibble_separated %>% dplyr::group_by(sitecode)
...
for (i in 1:length(grouped_site_list)) {
  ...
  # extract the years spanning the data 
  year <- unique(lubridate::year(data$DateTime))
  
  # create a warning message to indicate if data contains more than one calendar year
  if (length(year) > 1) {
    warning("Data spans more than one calendar year.")
  } else{
    
  }
  
  # construct a file name that uses the year from the data and the site code 
  filename <- paste0(paste(year[1], sitecodes_in_csv_files[i], sep = "_"), ".csv")
