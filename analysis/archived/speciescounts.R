...
setwd('C:/Users/Andrew/Dropbox/Horse_Research')
getwd()
...
## remember that to use the sum function you must first 
...
BGW18$horsecount <- speciestotal (BGW18, "Horse")
BGW18$cowcount <- speciestotal (BGW18, "Cow")
BGW18$elkcout <- speciestotal (BGW18, "Elk")
BGW18$deercount <- speciestotal (BGW18, "Muledeer")
BGW18$pronghorncount <- speciestotal (BGW18, "Pronghorn")
BGW18$humancount <- speciestotal (BGW18, "Person")
View(BGW18)

## some summary plots to visualize the data
plot(BGW18$horses)
hist(BGW18$horses)
barplot(BGW18$horses)

## create a new df
BGW18Species.df<- data.frame(species = c("horses", "cows", "elk", "muledeer", "pronghorn"), 
...
f <- ggplot(BGW18Species.df, aes(x = species, y = freq))
f + 
  geom_col(alpha = 1,
           color = "Black",
