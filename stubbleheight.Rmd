...
BKS18plot <- ggplot(BKS18stubblestats, aes(fill = Species, y = mean, x = Species)) +
  geom_col(width = 0.25) +
  ...
  # add the number of samples inside the bars
  annotate(geom = "text", x = 1, y = (BKS18stubblestats$mean[1]/2), 
...
```{r}
heber_stubble_data <- stubble_data %>% dplyr::filter(Site != "Boggy" | Site != "Wildcat")
```
