...
```{r View Factors Levles After Coding as Factors}
stubble_data %>% pull(Species) %>% levels()
```

Blue grama was coded as "BOGR2" and as "BOGRS" in this data set, and we want to have a single species code to represent blue grama. Therefore, anywhere "BOGRS" occurs in the data, we want to replace it with "BOGR2".
...
```{r View Factors Levels After Replacing "BOGRS" as "BOGR2"}
stubble_data %>% pull(Species) %>% levels()
```

Additionally, we want to collapse both Poa species (Poa pratensis and Poa fendleriana) into their genus (Poa). It can very difficulkt to differentiate these species while in the field, and lower confidence in our observations tends to make it more conservative (and safer) to collapse these species into a genus for analysis purposes. Functionally, both species are very similar.
...
View the species as factors indicates that there are a couple factors that we should re-code. The "Missing" factor will be re-coded as "Combined Species" as during that site visit we did not differentiate by species and combined all stubble height observations.

```{r View Factor Levels After Combining Poa}
stubble_data$Species %>% levels()
```


```{r Replace "Missing" Species with "Combined Species"}
stubble_data$Species <- 
  stubble_data$Species %>% str_replace(pattern = "Missing", 
                                     replacement = "Combined Species")
print(stubble_data)
```

We also will replace the annual forb species code "AAFF" with a more descriptive "Annual Forb" charater string. 
...
However, because we string replace some of the plant species, the "Species" column was converted into a character data type and the factors were dropped. We can then re-convert the "Species" column into factors and print the factor levels in the chunk.

```{r Recode Species as Factors}
stubble_data$Species <- stubble_data$Species %>% forcats::as_factor()
...
Plot a histogram bluegrass to visualize the distribution of stubble heights.
...
We will build a project-specific species list because many of the species are in common between sites, but fewer species are in common between projects, because the projects are in very different areas.

Drop the unused factor levels from the White Mountains data, and then re-order the factor levels by the frequency in which they occur.

```{r Drop Unused Levels White Mountains}
white_mountains_stubble$Species <- 
  white_mountains_stubble %>% 
  pull(Species) %>% 
  forcats::fct_drop() %>%
  forcats::fct_infreq()

white_mountains_stubble$Species %>% levels()
```
...
white_mountains_stubble_stats$Species %>% levels()
```
...
Drop the unused factor levels from the Heber data, and then re-order the factor levels by the frequency in which they occur.

```{r Drop Unused Levels Heber}
heber_stubble$Species <- 
  heber_stubble %>% 
  pull(Species) %>% 
  forcats::fct_drop() %>%
  forcats::fct_infreq()
heber_stubble$Species %>% levels()
```
...
```{r Check Factor Levels Heber Stubble Stats}
...
```{r Reorder Factor Levels Fifty One 2019}
FTO19stubblestats$Species <- 
  FTO19stubblestats$Species %>%
  forcats::fct_drop() %>%
  forcats::fct_inorder()
FTO19stubblestats$Species %>% levels()
```
...
```{r Reorder Factor Levels Black Canyon North 2019}
BKN19stubblestats$Species <- 
  BKN19stubblestats$Species %>%
  forcats::fct_drop() %>%
  forcats::fct_inorder()
BKN19stubblestats$Species %>% levels()
```
...
```{r Reorder Factor Levels Fifty One 2020}
FTO20stubblestats$Species <- 
  FTO20stubblestats$Species %>%
  forcats::fct_drop() %>%
  forcats::fct_inorder()
FTO20stubblestats$Species %>% levels()
```
...
```{r Reorder Factor Levels Black Canyon North 2020}
BKN20stubblestats$Species <- 
  BKN20stubblestats$Species %>%
  forcats::fct_drop() %>%
  forcats::fct_inorder()
BKN20stubblestats$Species %>% levels()
```
...
```{r Reorder Factor Levels Black Canyon South 2020}
BKS20stubblestats$Species <- 
  BKS20stubblestats$Species %>%
  forcats::fct_drop() %>%
  forcats::fct_inorder()
BKS20stubblestats$Species %>% levels()
```
