...
```{r}
#readr::write_csv(production, "data/vegetation/white-mountains/white-mountains-production-all-years-combined.csv")
```
...
```{r}
#readr::write_csv(residual, "data/vegetation/white-mountains/white-mountains-residual-all-years-combined.csv")
```
...
```{r}
# get summary stats on each site by year
stats_2017 <- production %>% 
  dplyr::filter(Year == 2017) %>% 
  dplyr::group_by(Site) %>% 
  get_summary_stats(lb_acre, type = "common")

stats_2018 <- production %>% 
  dplyr::filter(Year == 2018) %>% 
  dplyr::group_by(Site) %>% 
  get_summary_stats(lb_acre, type = "common")

stats_2019 <- production %>% 
  dplyr::filter(Year == 2019) %>% 
  dplyr::group_by(Site) %>% 
  get_summary_stats(lb_acre, type = "common")

stats_combined <- bind_rows(stats_2017, stats_2018, stats_2019)

# quick and dirty way to add year back into the data frame
# ideally, I would like to prevent the year from being removed
# one possible solution would be to filter by site and by year 
# store each value in an object and combine these objects together into a data frame

stats_combined<- stats_combined %>% 
  add_column(Year = c(2017, 2017, 2018, 2018, 2019, 2019), .after = "Site")
...
stats <- stats %>% bind_rows(BGW17, BGW18, BGW19, WCS17, WCS18, WCS19)
...
stats$max <- as.numeric(stats$max)
stats$median <- as.numeric(stats$median)
...
segments(0.7, stats$mean[1] - stats$se[1], 0.7, stats$mean[1] + stats$se[1], lwd = 1)
arrows(0.7, stats$mean[1]-stats$se[1], 0.7, stats$mean[1] + stats$se[1], lwd = 1, angle = 90, code = 3, length = 0.05)

segments(1.9, stats$mean[2] - stats$se[2], 1.9, stats$mean[2] + stats$se[2], lwd = 1)
arrows(1.9, stats$mean[2] - stats$se[2], 1.9, stats$mean[2] + stats$se[2], lwd = 1, angle = 90,code = 3, length = 0.05)

segments(3.1, stats$mean[3] - stats$se[3], 3.1, stats$mean[3] + stats$se[3], lwd = 1)
arrows(3.1, stats$mean[3] - stats$se[3], 3.1, stats$mean[3] + stats$se[3], lwd = 1, angle = 90,code = 3, length = 0.05)

segments(4.3, stats$mean[4] - stats$se[4], 4.3, stats$mean[4] + stats$se[4], lwd = 1)
arrows(4.3, stats$mean[4] - stats$se[4], 4.3, stats$mean[4] + stats$se[4], lwd = 1, angle = 90,code = 3, length = 0.05)

segments(5.5, stats$mean[5] - stats$se[5], 5.5, stats$mean[5] + stats$se[5], lwd = 1)
arrows(5.5, stats$mean[5] - stats$se[5], 5.5, stats$mean[5] + stats$se[5], lwd = 1, angle = 90,code = 3, length = 0.05)

segments(6.7, stats$mean[6] - stats$se[6], 6.7, stats$mean[6] + stats$se[6], lwd = 1)
arrows(6.7, stats$mean[6] - stats$se[6], 6.7, stats$mean[6] + stats$se[6], lwd = 1, angle = 90,code = 3, length = 0.05)
