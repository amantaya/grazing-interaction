...
```{r}
stats<- data.frame(sites=c("BGW17", "BGW18", "BGW19", "WCS17", "WCS18", "WCS19"), 
                   avg_grams_meter = NA, 
...
stats_rstatix <- stats_rstatix %>% bind_rows(BGW17, WCS17, BGW18, WCS18, BGW19, WCS19)

# add the site names to the data frame 
stats_rstatix <- stats_rstatix %>% 
  add_column(site = c("BGW17", "WCS17", "BGW18", "WCS18", "BGW19", "WCS19"), .before = 1)
...
Convert the grams per square meter to other units, kilograms per hectare and pounds per acre.
```{r}
...
# convert the S3 units class to numeric to prevent future problems
...
Test the standard deviation and standard error functions on a single site and year before applying it to all of the data. Check to see if the values are correct.
```{r}
...
```{r}
## calculate standard deviation SD for each year and site (in lb_acre)
...
```
```{r}
# there is a better way to calculate the confidence interval than by using a t value
# reason we don't want to do is this way is because the degrees of freedom changes between years
# TODO overwrite the stats dataframe using the get_summary_stats() function
stats$L95 <- stats$avg_lbs_acre-2.776*stats$se
stats$U95 <- stats$avg_lbs_acre+2.776*stats$se
print(stats)
```
