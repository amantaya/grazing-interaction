...
```{r}
## read in csv data
heber_prod_group1 <- readr::read_csv('data/vegetation/heber/production/csv/2021-heber-production-group1.csv',
                                col_names = TRUE, 
                                na = c(""," ","NA"))

heber_prod_group2 <- readr::read_csv('data/vegetation/heber/production/csv/2021-heber-production-group2.csv',
                                col_names = TRUE,
                                na = c(""," ","NA"))

heber_prod_group3  <- readr::read_csv('data/vegetation/heber/production/csv/2021-heber-production-group3.csv',
                                col_names = TRUE,
                                na = c(""," ","NA"))
...
```{r}
print(heber_prod_group1)
print(heber_prod_group2)
print(heber_prod_group3)
```

```{r}
heber_prod_group1$Date <- lubridate::ymd(heber_prod_group1$Date)
heber_prod_group2$Date <- lubridate::mdy(heber_prod_group2$Date)
heber_prod_group3$Date <- lubridate::mdy(heber_prod_group3$Date)
```

```{r}
heber_prod_2021 <- dplyr::bind_rows(heber_prod_group1, 
                                    heber_prod_group2, 
                                    heber_prod_group3)

print(heber_prod_2021)
```


```{r}
slash_s_data <- heber_prod_2021 %>% filter(Type == "CY")
...
```{r}
slash_s_data <- slash_s_data %>% dplyr::rename(Rank = Plot)

print(slash_s_data)
```

```{r}
slash_s_data$`Site Code` <- gsub("KA8", "KA-8", slash_s_data$`Site Code`)
...
```{r}
slash_s_data <- slash_s_data %>% 
  dplyr::group_by(`Site Code`) %>% 
...
```{r}
write.csv(slash_s_data, "data/vegetation/2021-11-18-slash-s-comparative-yield-bag-weights.csv")
```
