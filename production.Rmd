...
```{r}
# TODO instead of reading in "TotalDry" calculate it from the columns
# TODO rename the "TotalDry" column to "TotalDryWt"
print(wm_prod_2017)
...
Force conversion of the 2019 "Plot" column to a character.
```{r}
...
There is an error in our 2019 data, however, that we need to adjust. 
For Wildcat, instead of clipping the entire 1m X 1m grazing exclosure, we clipped three 40cm x 40 cm quadrats randomly placed inside of the exclosure. We decided to clip a smaller area to save time.
This differs from how we clipped the Boggy Creek site for the same year. For Boggy, we clipped the entire 1m x 1m exclosure.

To correct this error, first let's calculate the area of one 40cm x 40 cm quadrat.
```{r}
...
```
Next, find the total area of three 40cm x 40cm quadrats in centimeters[^2].
```{r}
...
wm_prod_2019$AdjustedWt <- ifelse (wm_prod_2019$Site == "Wildcat" & wm_prod_2019$Type == "Exclosure", 
                           wm_prod_2019$TotalDry*conversion, 
                           wm_prod_2019$TotalDry)
...
wm_prod_2017$AdjustedWt <- dplyr::if_else (wm_prod_2017$Type == "Residual", 
                                   wm_prod_2017$TotalDry*conversion, 
                                   wm_prod_2017$TotalDry)

wm_prod_2018$AdjustedWt <- dplyr::if_else (wm_prod_2018$Type == "Residual", 
                                   wm_prod_2018$TotalDry*conversion, 
                                   wm_prod_2018$TotalDry)
