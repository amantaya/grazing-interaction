...
```{r Load Data}
heber_production_group1 <- readr::read_csv(
  file.path("data",
            "production",
            "heber",
            "csv",
  ...
  col_names = TRUE, 
  na = c(""," ","NA"))
print(heber_production_group1)

heber_production_group2 <- readr::read_csv(
  file.path("data",
            "production",
            "heber",
            "csv",
  ...
  col_names = TRUE, 
  na = c(""," ","NA"))
print(heber_production_group2)

heber_production_group3 <- readr::read_csv(
  file.path("data",
            "production",
            "heber",
            "csv",
...
heber_production_group2$Date <- heber_production_group2$Date %>% 
  lubridate::mdy()
print(heber_production_group2)
heber_production_group3$Date <- heber_production_group3$Date %>% 
  lubridate::mdy()
print(heber_production_group3)
```
...
heber_production_2021 <- dplyr::bind_rows(heber_production_group1,
                                          heber_production_group2,
                                          heber_production_group3)
...
dlookr::diagnose(heber_production_2021)
```
...
```{r Unique Site Names}
heber_production_2021 %>% dplyr::pull(Site) %>% unique()
```

Same thing for site codes. 

```{r Unique Site Codes}
heber_production_2021 %>% dplyr::pull(`Site Code`) %>% unique()
```
...
slash_s_data <- heber_production_2021 %>% 
...
heber_production_2021[heber_production_2021$Site == "Area 51" & 
                        is.na(heber_production_2021$SubSample) == TRUE &
                        heber_production_2021$Type == "Exclosure", ] <- 
heber_production_2021[heber_production_2021$Site == "Area 51" & 
                        is.na(heber_production_2021$SubSample) == TRUE &
                        heber_production_2021$Type == "Exclosure", ] %>% 
...
heber_production_2021 %>% 
  ungroup() %>% 
  mutate(Index = (1:nrow(heber_production_2021)), .before = Site)
print(heber_production_2021)
```
...
heber_production_2021$SubSample[82:85]
```

Replace those values with the correct values.

```{r replace duplicate "SubSample" values}
heber_production_2021$SubSample[82:85] <- c(1, 2, 3, 4)
```
...
only_ponderosa_plot5 <- heber_production_2021 %>% 
...
```{r}
heber_production_2021$SubSample[7:9]
```

```{r}
heber_production_2021$SubSample[7:9] <- c(1, 2, 3)
```
...
```{r}
heber_production_2021 %>% ungroup() %>% select(c("Index", "Site", "Plot", "SubSample"))
```

```{r}
heber_production_2021$SubSample
```

```{r}
heber_production_2021 %>% group_by(Site, PlotType, Plot) %>% group_keys()
```
...
heber_production_2021 %>% 
  group_by(Site, PlotType, Plot) %>% 
  dplyr::add_count(Plot)
```

write an if else inside of for loop
if its the same plot, add 1 to the SubSample number
if its a different plot, start at 1

```{r}
heber_production_2021_exclosures <- heber_production_2021 %>% ungroup() %>% dplyr::filter(PlotType == "Exclosure")

print(heber_production_2021_exclosures)
```

```{r}
heber_production_2021_subsample <- NULL

for (i in 1:nrow(heber_production_2021_exclosures)){
  
  while(i < nrow(heber_production_2021_exclosures)){
    previous_row <- heber_production_2021_exclosures[i, ]
    
    next_row <- heber_production_2021_exclosures[i+1, ]
    
    if ((next_row$Site == previous_row$Site & next_row$Plot == previous_row$Plot)){
      next_row$SubSample <- previous_row$SubSample + 1
      } else {
        next_row$SubSample <- 1
      }
    heber_production_2021_subsample[i] <- next_row
    } 
  }
```

```{r}
heber_production_2021_exclosures %>% dplyr::group_by(Site, Plot) %>% dplyr::group_size()
```
...
heber_production_2021 %>% 
  group_by(Site, PlotType, Plot) %>% 
  dplyr::add_count(Plot) %>% 
  mutate(SubSample = dplyr::if_else(n == 1, # condition
                                    1, # if true
                                    (1:n), # if false
                                    NA_character_) # if missing
  )
```


```{r write cleaned data to csv}
write_csv(heber_production_2021,
          file.path(currentwd,
                    "data",
                    "production",
                    "heber",
