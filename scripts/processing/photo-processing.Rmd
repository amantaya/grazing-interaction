...
col_spec <- cols(
  ...
  ImageTime = col_time(format = ""),
  ImageDate = col_date(format = ""),
...
short_names <- str_extract(long_names, 
                           pattern = ("\\d{4}_[:upper:][:upper:][:upper:]")
                           )

print(short_names)
```

# TODO test if will work with cameratraps2

```{r rename elements in list of csv files}
long_names <- names(csv_files)

short_names <- stringr::str_extract(long_names, 
                           pattern = "\\d{4}_[[:upper:]]..|[[:upper:]][[:upper:]][[:upper:]]\\d\\d")

names(csv_files) <- short_names
...
```{r get sites from sitecodes}

csv_files_sitecodes <- short_names <- stringr::str_extract(long_names, 
                           pattern = "[[:upper:]]..|[[:upper:]][[:upper:]][[:upper:]]")

sites <- sitecode_constructor(csv_files_sitecodes)
...
```{r}
comments_df %>% pull(Comments) %>% unique() %>% tibble()
```

# TODO I will copy these photos into a separate folder for manual review
...
csv_files_dropped_no_detections <- 
...
```{r arrange by site name and convert to data frame}
all_photo_data <- purrr::map_dfr(csv_files_dropped_no_detections,
                                 dplyr::arrange, 
                                 Site)
...
```{r convert date and time to strings}

all_photo_data$ImageDate <- strftime(all_photo_data$ImageDate, format = "%F")

all_photo_data$ImageTime <- strftime(all_photo_data$ImageTime, format = "%T")
```
...
```{r write to csv}
readr::write_csv(all_photo_data, 
                       file.path(currentwd, 
                                 "data", 
                                 "photo", 
                                 "all_photo_data.csv"))
