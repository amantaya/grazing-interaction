...
As we mentioned, we sampled the entire 1 m[^2] plot at the Only Ponderosa site. We will create a new column called `SampleArea` which represents the area sampled for each exclosure at each site. Here we set the units for the `SampleArea` column for the "Only Ponderosa" site, using 1 m[^2], because if we didn't, it would greatly overestimate the amount of production at the Only Ponderosa site. All other sites were set to 0.48 m[^2]. 
...
Calculate summary statistics for exclosure plots summarizing by 
...
Method 2 calculates are average for each exclosure plot by averaging the area sampled by 3 sub-plots. This will likely create more error because we are using an sub-plot average to then generate an estimate for each plot, where we average plots together to generate a production estimate for each site.

I am performing this method as an experiment to understand how the error compounds.

```{r set units for each sub-plot}
...
```{r create "SampleArea" column for each sub-plot}
heber_production_2021 <- heber_production_2021 %>% 
  dplyr::filter(PlotType == "Exclosure") %>% 
...
```{r add "GramsPerMeter" column for each sub-plot}
heber_production_2021 <- 
  heber_production_2021 %>% mutate(GramsPerMeter = TotalDryWt / SampleArea)
print(heber_production_2021)
```

```{r convert "GramsPerMeter" to "LbsPerAcre"}
heber_production_2021 <- heber_production_2021 %>% 
