...
heber_production_2021_method1_site_summary <- 
heber_production_2021_method1 %>% 
  group_by(Site) %>% 
  rstatix::get_summary_stats(GramsPerMeter, 
                             type = "common", 
                             show = c("n","mean","sd","ci"))
print(heber_production_2021_method1_site_summary)
```
...
heber_production_2021_method2_site_summary <- 
  heber_production_2021_method2_site_summary %>% 
  dplyr::mutate(mean = set_units(mean, lb/acre)) %>% 
  dplyr::mutate(sd = set_units(sd, lb/acre)) %>% 
  dplyr::mutate(ci = set_units(ci, lb/acre))
...
heber_production_2021_method1_site_summary %>% dplyr::select(-c("variable", "n"))
```

Display the results from Method 2.

```{r print results method 2}
heber_production_2021_method2_site_summary %>% dplyr::select(-c("variable", "n"))
```

We can also display the differences by subtracting the results from Method 1 from the results from Method 2.

```{r}
heber_production_2021_method1_site_summary$mean - heber_production_2021_method2_site_summary$mean
```

```{r}
heber_production_2021_method1_site_summary$sd - heber_production_2021_method2_site_summary$sd
```

```{r}
heber_production_2021_method1_site_summary$ci - heber_production_2021_method2_site_summary$ci
```
