...
```{r Photo: Map of Study Area, echo=FALSE, fig.width=6.5, fig.height=5, fig.cap="Location of Motion Cameras (Blue), Time-lapse Cameras (Red), and Addtional Vegetation Monitoring Sites (Green) within the Heber Wild Horse Territory (HWHT) and on adjacent lands of the Apache-Sitgreaves National Forests, Black Mesa Ranger District."}
knitr::include_graphics("docs/pubs/images/2021-Heber-Camera-and-Vegetation-Sites-University-of-Arizona.jpg")
```
...
```{r Heber Veg Sites Summary Table, echo=FALSE, fig.cap="Vegetation monitoring sites were established in key areas identified by USFS range management personnel and were selected by a stratified random design based on Terrestrial Ecosystem Unit. Sites with time-lapse cameras also received 5 grazing exclosures for the paired-plot method. At all vegetation monitoring sites, we measured species canopy cover i.e. Daubenmire and landscape appearance." }
print(heber_veg_sites)
```
...
```{r Photo: Grazing Exclosures at Bear, echo=FALSE, fig.height=5, fig.width=6.5, fig.cap="Example of three grazing exclosures at the Bear site, July 2021."}
knitr::include_graphics("docs/pubs/images/2021-bear-grazing-exclosures.JPG")
```
...
heber_paired_plot_2021_site_summary %>% 
  ...
  mutate(SampleArea = dplyr::if_else(Site == "Only Ponderosa",
                                     ...
                                     NA_character_)) %>% 
  dplyr::select(-c("variable", "mean", "sd", "ci")) %>% 
  dplyr::mutate(n = as.character(n)) %>% 
  dplyr::rename("Number of Exclosure Plots" = n) %>% 
  dplyr::rename("Clipped Sample Area" = SampleArea)
```

```{r Heber Paired Plot Site Summary Table, echo=FALSE, tab.cap="Number of Exclosure Plots Clipped at Each Site"}
...
print(n_exclosure_plots_clipped)
```
...
to our right toes, and each point was separated by 1 pace. We then
...
```{r Photo: Monitoring GETPBG, echo=FALSE, fig.height=5, fig.width=6.5, fig.cap="Monitoring At the GETPBG site"}
knitr::include_graphics("docs/pubs/images/monitoring-GETPBG.jpg")
```
